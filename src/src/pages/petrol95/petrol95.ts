...
@IonicPage()
@Component({
  selector: 'page-petrol95',
    templateUrl: 'petrol95.html',
    providers: [LaunchNavigator]
})
export class Petrol95Page {
  destination: any;
  ...
  displayGoogleMap() {
    this.geolocation.getCurrentPosition().then((position) => {
      ...
      this.http.get('https://raw.githubusercontent.com/vidura/CPC_mobile/json/results.json').map((res) => res.json()).subscribe(data => {
        this.addMarkersMap95(data, latlong);
      });
  ...
  addMarkersMap95(markers, lat) {
    ...
    for (let marker of markers) {
      ...
      for (i = 0; i < products.length; i++) {
        if (products[i] == "95Petrol" && type == "CPC_dealer") {
